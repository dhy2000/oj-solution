...
inline static ll wrap(ll i, ll n) {
    return (i + n) % n;
}
...
int main() {
    ...
    while (scanf("%d %d", &m, &n) != EOF) {
        static ll mat[105][105], ans[105][105];
        static int nxt[105][105];
        FOR_RANGE_LT(i, 0, m)
            FOR_RANGE_LT(j, 0, n)
                ans[i][j] = mat[i][j] = getint();
        FOR_REV_EQ(j, n - 2, 0) {
            FOR_RANGE_LT(i, 0, m) {
                ans[i][j] = ans[i][j + 1];
                nxt[i][j] = i;
                ...
                if (update_min(ans[i][j], ans[i1][j + 1])) {
                    nxt[i][j] = i1;
                } else if (ans[i1][j + 1] == ans[i][j] && i1 < nxt[i][j])
                    nxt[i][j] = i1;
        ...
        FOR_RANGE_LT(i, 0, m)
            FOR_RANGE_LT(j, 0, n)
                printf("%lld(%d)%c", ans[i][j], nxt[i][j] + 1, j + 1 == n ? '\n' : ' ');
#endif
        ll mpath = ans[0][0], mfirst = 0;
        FOR_RANGE_LT(i, 1, m)
            if (update_min(mpath, ans[i][0]))
                mfirst = i;
        static int route[105];
        int p = mfirst;
        FOR_RANGE_LT(j, 0, n) {
            route[j] = p + 1;
            p = nxt[p][j];
