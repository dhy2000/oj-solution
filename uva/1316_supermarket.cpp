...
struct Prod {
    int profit, ddl;
    Prod(int profit, int ddl): profit(profit), ddl(ddl) {}
...
int main() {
    ...
    int n;
    while (scanf("%d", &n) > 0) {   // something wrong with `!= EOF`
        assert(0 <= n && n <= N);
        vector<Prod> prods;
        FOR_RANGE_LT(i, 0, n) {
            int profit = getint(), ddl = getint();
            assert(1 <= profit && profit <= N);
            assert(1 <= ddl && ddl <= N);
            prods.push_back(Prod(profit, ddl));
        }
        std::sort(prods.begin(), prods.end(), cmp);
        int fast = 0, slow = 0;
        
        priority_queue<Prod> pq;
        ll sum = 0;
        while (slow < n) {
            while (fast < n && prods[fast].ddl == prods[slow].ddl) ++fast;
            // [slow, fast) has same ddl
            int days = prods[slow].ddl - (fast >= n ? 0 : prods[fast].ddl);
            for (; slow < fast; ++slow)
                pq.push(prods[slow]);
            for (; days && !pq.empty(); --days) {
#ifdef DEBUG
                fprintf(stderr, "Choose [%d, %d]\n", pq.top().profit, pq.top().ddl);
#endif
                sum += pq.top().profit;
                pq.pop();
            }
