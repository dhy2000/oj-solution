...
int main() {
    ...
    while (1) {
        ...
        if (n == 0 && m == 0)
            break;
        static int dragons[N], knights[N];
        FOR_RANGE_LT(i, 0, n)
            dragons[i] = getint();
        FOR_RANGE_LT(i, 0, m)
            knights[i] = getint();
        ...
        ll ans = 0;
        FOR_RANGE_LT(i, 0, n) {
            for (; cur_knight < m && knights[cur_knight]  < dragons[i]; cur_knight++);
            if (cur_knight < m) {
                assert(knights[cur_knight] >= dragons[i]);
                ans += knights[cur_knight];
                cur_knight++;
            } else {
                // the tallest knight cannot defeat the largest dragon's head
                solved = false;
                break;
            }
        }
