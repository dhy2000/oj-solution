...
inline static bool is_palindrome(const char *base, size_t start, size_t end) {
    FOR_RANGE_EQ(i, start, end)
        if (base[i] != base[end + start - i])
            return false;
    return true;
}
...
int main() {
    ...
    while (T--) {
        ...
        FOR_RANGE_LT(i, 0, len) {
            ...
            FOR_REV_EQ(j, i, 0) {
                // 0..j, j..=i
                if (is_palindrome(txt, j, i))
                    update_min(cut[i], (j - 1 >= 0 ? cut[j - 1] : 0) + 1);
            }
        }
        printf("%d\n", cut[len - 1]);
    }
